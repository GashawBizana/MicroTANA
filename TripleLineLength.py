...
class NLPCA(object):
    ...
    def create_model(self, num_dim, nodes, lr):
        ...
        gradient_descent = tf.keras.optimizers.Adam(learning_rate=lr)
...
# z=np.cos(t) + np.random.normal(scale = .1, size = 1000)
...
data_new = pca.preprocess( thinned_points )

# fit the data
pca.fit(data_new, epochs = 1000, nodes = 5, lr = .01, verbose = 0)
...
ax2.scatter(data_new[:,0], 
            data_new[:,1],data_new[:,2], 
            s = 5, 
            c = proj.reshape(-1), 
            cmap = 'viridis')
ax2.plot(curve_pts[:,0], 
          curve_pts[:,1],curve_pts[:,2], 
           color = 'black',
           linewidth = '2.5')
